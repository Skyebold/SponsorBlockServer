...
import { VideoDBSegment } from "../../src/types/videoSegments.model";
...
describe("downvoteSegmentArchiveJob", () => {
    ...
    const getArchivedSegment = (): Promise<VideoDBSegment[]> => db.prepare("all", 'SELECT * FROM "archivedSponsorTimes"');

    const getSegmentsInMainTable = (dayLimit: number, voteLimit: number, now: number): Promise<VideoDBSegment[]> =>
        db.prepare(
            "all",
